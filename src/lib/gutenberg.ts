...
export async function fetchAndPaginateGutenbergBook(gutenbergId: string | number) {
  ...
  if (!textUrl) {
    throw new Error("No readable text format available for this book");
  }
...
function paginateText(text: string, charsPerPage: number): Record<number, string> {
  ...
  for (const paragraph of paragraphs) {
    ...
    if (currentContent.length + trimmed.length > charsPerPage && currentContent.length > 0) {
