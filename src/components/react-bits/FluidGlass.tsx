...
export default function FluidGlass({
  ...
  const getModeSelection = (m: Mode) => {
    if (m === "bar") return { Wrapper: Bar, overrides: barProps };
    if (m === "cube") return { Wrapper: Cube, overrides: cubeProps };
    return { Wrapper: Lens, overrides: lensProps };
  };

  const { Wrapper, overrides: rawOverrides } = getModeSelection(mode);
...
const ModeWrapper = memo(
  ({
    ...
    useFrame((state, delta) => {
      ...
      const getDestY = () => {
        if (lockToBottom) return -v.height / 2 + 0.2;
        if (followPointer) return (pointer.y * v.height) / 2;
        return 0;
      };

      const destX = followPointer ? (pointer.x * v.width) / 2 : 0;
      const destY = getDestY();
