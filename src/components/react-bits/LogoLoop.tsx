...
const useAnimationLoop = (
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [targetVelocity, seqWidth, seqHeight, isHovered, hoverSpeed, isVertical, isInView]);
};
...
export const LogoLoop = memo<LogoLoopProps>(
  ({
    ...
    const getContainerWidth = useCallback(() => {
      if (isVertical) {
        const cssLen = toCssLength(width);
        return cssLen === "100%" ? undefined : cssLen;
      }
      return toCssLength(width) ?? "100%";
    }, [isVertical, width]);
    ...
    const containerStyle = useMemo(
      ...
      [getContainerWidth, cssVariables, style],
    );
