...
const FuzzyText: React.FC<FuzzyTextProps> = ({
  ...
  const isVisible = useInView(canvasRef, { amount: 0 });
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      let lastFrameTime = 0;
      // Use lower FPS (20) when idle to free frame budget for scroll
      const idleFps = Math.min(fps, 20);
      const activeFps = fps;
      const getFrameDuration = () =>
        1000 / (isHovering || isClicking || isGlitching ? activeFps : idleFps);
      ...
      const run = (timestamp: number) => {
        ...
        if (transitionDuration > 0) {
          const step = 1 / (transitionDuration / getFrameDuration());
