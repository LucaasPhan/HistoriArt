...
async function main() {
  try {
    ...
    const content = fs.readFileSync(path.join(__dirname, "migrations", "0009_open_makkari.sql"), "utf-8");
    ...
    for (let i = 0; i < statements.length; i++) {
      ...
      try {
        ...
        } else if (e.message?.includes('column "chapter_number" of relation "quiz_questions" already exists')) {
